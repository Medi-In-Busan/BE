...
/** CLOVA OCR 원본 응답을 그대로 넘기지 않고, 서비스에서 필요한 텍스트만 추출해 앱 전용 DTO로 변환한다. */
public final class DocumentOcrDtoMapper {

    private DocumentOcrDtoMapper() {
    }

    public static DocumentOcrResponse toResponse(ClovaOcrResponse clovaResponse) {
        ...
        return new DocumentOcrResponse(joinFields(image.fields()));
    }
