...
class DocumentOcrControllerTest {
    ...
    void 이미지를_업로드하면_추출된_텍스트를_반환한다() throws Exception {
        ...
        when(documentOcrService.extractText(any())).thenReturn(new DocumentOcrResponse("환자명 홍길동"));
    ...
    void 유효성_예외가_발생하면_400과_에러코드를_반환한다() throws Exception {
        ...
        when(documentOcrService.extractText(any())).thenThrow(new InvalidDocumentImageException("지원하지 않는 이미지 형식입니다."));
    ...
    void CLOVA_OCR_실패시_지정된_상태코드와_고정_메시지를_반환한다() throws Exception {
        ...
        when(documentOcrService.extractText(any())).thenThrow(new DocumentOcrFailedException(HttpStatus.BAD_GATEWAY));
    ...
    void 예상치_못한_오류는_500과_고정_메시지를_반환한다() throws Exception {
        MockMultipartFile image = new MockMultipartFile("image", "doc.jpg", "image/jpeg", new byte[]{1});
        when(documentOcrService.extractText(any())).thenThrow(new RuntimeException("boom"));
