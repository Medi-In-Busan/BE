...
/**
 * 그 장소를 다녀오는 데 몸이 드는 부담의 정도. 값이 클수록 뒤로 미루는 게 좋다.
 *
 * ⚠️ **"언제부터 가도 된다"는 허가가 아니다.** 앱은 시술 종류도 환자 상태도 모르고 장소 유형만
 * 보므로, 특히 부담이 가장 적은 [IMMEDIATE]를 "진료 당일도 괜찮다"로 표시하면 앱이 할 수 없는
 * 의료적 판단을 한 셈이 된다(같은 프로필의 RESTAURANT가 [PlaceCautionKey.FASTING] 주의를 달고
 * 있는 것과도 앞뒤가 맞지 않았다). 표시 문구는 장소 쪽 부담만 말하고, 시기 판단은 의료기관
 * 안내로 넘긴다 — PlaceCurationStrings.atAGlanceSourceNote가 그 한 줄을 항상 함께 띄운다.
 */
enum class RecoveryFit {
    /** 부담이 가장 적다(짧은 실내 체류·앉아서 쉬는 활동). */
    IMMEDIATE,
