 ...
 * 그런 표기가 통째로 안 걸려 평문 그대로 노출된다. 길이를 3으로 묶어 `" - "`까지는 받는다.
 *
 * 공백은 가로 공백(`\h`)만 받는다 — `\s`로 두면 개행까지 구분자가 되어, 줄 끝의 6자리와 다음 줄
 * 첫 7자리처럼 **서로 무관한 숫자가 주민번호로 오인**될 뿐 아니라 치환하면서 그 줄바꿈까지 먹어
 * 두 줄이 한 줄로 합쳐진다(문서 배치가 통째로 어긋난다).
 */
private const val NumberSeparator = "[-.\\h]{0,3}"
