 ...
 * 받아 조용히 [OTHER]로 떨어진다.
 *
 * 지금은 쇼핑과 음식 하위가 채워진다. 음식 세분화는 지도(S-08)의 "음식" 탭 필터가 쓴다 — 그 탭은
 * 전부 같은 [PlaceType.RESTAURANT]라 이 분류가 없으면 걸러낼 기준 자체가 없다. 다만 음식점은
 * TourAPI 말고 부산맛집정보(cat3 없음)에서도 들어와서, 그쪽에서 온 장소는 계속 [OTHER]로 남는다.
 *
 * 분류의 근거가 되는 TourAPI cat3 코드는 백엔드가 장소 종류와 무관하게 전부 저장하므로,
 * 관광지·숙박 세분화가 필요해지면 양쪽 enum과 백엔드 매핑만 늘리면 된다.
 */
enum class PlaceCategory {
     ...
     * (부산맛집정보의 음식점 등), 그리고 아직 세분화 대상이 아닌 종류(관광지·숙박)가 전부 여기로
     * 온다 — 즉 **기본값이자 대다수**다. 화면에서는 이 값일 때 세부 분류를 표시하지 않고 [PlaceType] 라벨로 되돌아간다.
     */
    OTHER
}
